...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mkdir([defaultpath '\data\Testdata']);
addpath(genpath([defaultpath '\functions']))

NoImage=10;
RecDuration=4;%sec ; time limit of 1 trial %
numrep=5;
conditions = {'Brain', 'Manual', 'Brain+Manual'};
condorder= randperm(length(conditions));

%%sub
savefilename=[defaultpath '\data\Testdata\' subName  '-' datestr(now,30)];
...
for C=1:length(conditions)
    ccond=conditions{condorder(C)};
    outcsvname=[folderPath '_' num2str(C) '\history.csv' ];

    %% ready
    DrawFormattedText(w,double([num2str(C) '回目　画像の好みを回答してください。\n波形がきれいになったらクリックして回答スタート']), 'center', 'center',  WhiteIndex(w));
    Screen('Flip', w); %
    ...
    for R=1:numrep
        while 1
            imgpath=[folderPath '_' num2str(C)  '\iter_' num2str(R)];
            IMGlist=dir([imgpath '\*.jpg']);
            imglist={IMGlist.name};
            if isempty(imglist)
                DrawFormattedText(w,double([num2str(C)  ' iter_' num2str(R) '画像生成中']), 'center',50,  WhiteIndex(w));
                Screen('Flip', w); %
                WaitSecs(.1);
            elseif length(imglist)==10
                break
            end
        ...
        while 1
            outcsv = readtable(outcsvname);
            rowIdx = find(outcsv.iter_num == R);
            DrawFormattedText(w,double([num2str(C)  ' iter_' num2str(R) 'CSV確認中']), 'center',50,  WhiteIndex(w));
            Screen('Flip', w); %
        ...
        for T=1:length(imglist)
            ...
            OKFlag=0;decodedstate=[];NoiseFlagLog=[]; NoiseLog=[];TimeLog=[];
            while 1
                Screen('DrawTexture', w, A_tex, [], destRect);
                DrawFormattedText(w,double([num2str(C) '-' num2str(R) '-' num2str(T) '/' num2str(length(stimorder))  '　好みを回答してください(赤くなったら回答可能)']), 'center',50,  WhiteIndex(w));
                %% VAS
                ...
                if length(alldataV) > SAMPLE_FREQ_VIE*timeWindow && ctime>nextdec
                    ...
                    cpower=[pxx(:,3)];%1Trial 2windowNo 3freq 4channel
                    ...
                    predictV=nansum(cVIEscorez'.* VIE2preferenceModel.CoefMat(1:end-1))+VIE2preferenceModel.CoefMat(end);
                    ...
                    if sum(validIdx) >= 5
                        cdecresult=mean(decodedstate(~NoiseFlagLog,:));
                        OKFlag=1;
                    end
            ...
            %% save DataTest
            DataTest{TrialNo,1} =ccond;
            DataTest{TrialNo,2} =R;
            DataTest{TrialNo,3} =T;
            DataTest{TrialNo,4} =imglist{cstim};
            DataTest{TrialNo,5} = alldataV;
            DataTest{TrialNo,6} = PercentAns;
            DataTest{TrialNo,7} = NoiseFlagLog;
            DataTest{TrialNo,8} = NoiseLog;
            DataTest{TrialNo,9} = TimeLog;
            DataTest{TrialNo,10} = decodedstate;
            DataTest{TrialNo,11} =cdecresult;

            ctime=toc(starttime);
            rowIdx = find(outcsv.iter_num == R & outcsv.id == T-1);

            if strcmp(ccond,'Manual')
                new_value=PercentAns;
            elseif strcmp(ccond,'Brain')
                new_value= cdecresult;
            elseif strcmp(ccond,'Brain+Manual')
                new_value= [PercentAns+cdecresult]./2;
            end
            new_value=max(new_value,0);
            new_value=min(new_value,100);

            outcsv{rowIdx, 4} = new_value;
            cprompt = outcsv{rowIdx, 3};

            DataTest{TrialNo,12} =cprompt;

            DataTest{TrialNo,13} =new_value;


            TrialNo=TrialNo+1;%次のトライアルへ
        end
        writetable(outcsv, outcsvname);    % 5. 更新したテーブルをCSVとして再出力（保存）
        fid=fopen(outcsvname,'rt');
        fclose(fid);
    end
    save([savefilename 'DataTest.mat'],'Data','DataTest','subName','selpath','imglist','IMGlist','Question','ResultPre','SAMPLE_FREQ_VIE','NoiseMetricsLabel',...
        'NoImage', 'imgpath', 'stimPath','T','savefilename',...
        '-v7.3');
end
