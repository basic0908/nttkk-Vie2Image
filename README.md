...
本プロジェクトでは、画像生成モデルに伴う高負荷な処理をクラウドにオフロードするため、クラウド（AWS）とローカル環境を連携させたデュアル環境システムを構築しています。両環境間の通信にはFastAPIを使用しています。

LLMによるプロンプト生成、EC2へのデータ送信、Fluxモデルによる画像生成（10枚）、そしてローカル環境への返送という一連のプロセスは、**約1分**で完了します。
...
## 💻 使用方法
ローカル環境で`ec2-client.py`を実行すると、GradiによるWeb UIがローカルサーバーが(ポート7860)で立ち上がります。
